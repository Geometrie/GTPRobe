...
CGameBoardManager::CGameBoardManager()
{
	...
	m_emBlankMove.test = false;
	m_lpemCurrentMove = &m_emBlankMove;
	m_nHandicap = 0;
	m_iHandicapPutting = 0;
	m_lpemLivingMove = m_lpemCurrentMove;
}
...
bool CGameBoardManager::OnAddMove(int x, int y)
{
	...
	else
	{
		...
		if (m_lpemCurrentMove != NULL)
		{
			...
			{
				while (lpemMoveVisitor->longer != NULL && lpemMoveVisitor->longer->test && lpemMoveVisitor->longer->depth < lpemMoveVisitor->depth)
				{
				...
				if (lpemMoveVisitor->parent->test && iBranchDepth > lpemMoveVisitor->parent->depth)
				{
...
bool CGameBoardManager::OnRedoMove(CGameBase::ExtendMove *lpemMove)
{
	...
	if (lpemMove == NULL)
	{
		if (m_lpemCurrentMove == m_lpemLivingMove && !m_qubmGTPMoves.empty())
		{
			m_scTurnColor = m_qubmGTPMoves.front().stone_color;
			x = m_qubmGTPMoves.front().x;
			y = m_qubmGTPMoves.front().y;
			m_qubmGTPMoves.pop();
			lpemMove = m_lpemLivingMove->search(x, y);
			if (lpemMove != NULL)
			{
				m_fnExcuteRedo(lpemMove);
				while (lpemMove->longer != NULL)
				{
					m_fnSwapLonger(lpemMove);
				}
				m_lpemLivingMove->child = lpemMove;
			}
			else
			{
				lpemMove = m_fnExecuteMove(x, y);
				lpemMove->parent = m_lpemLivingMove;
				lpemMove->shorter = m_lpemLivingMove->child;
				if (lpemMove->shorter != NULL)
				{
					lpemMove->shorter->longer = lpemMove;
				}
				m_lpemLivingMove->child = lpemMove;
				++(m_lpemLivingMove->branch);
			}
			lpemMove->test = false;
			lpemMove->depth = 0;
			m_lpemLivingMove = lpemMove;
			m_lpemCurrentMove = m_lpemLivingMove;
			lpemMove->step = lpemMove->parent->step + 1;
			while (lpemMove->parent != NULL)
			{
				lpemMove->parent->depth = lpemMove->depth + 1;
				lpemMove = lpemMove->parent;
			}
			bSucceed = true;
		}
		else
...
void CGameBoardManager::OnClearGameRecord()
{
	...
	m_lpiPrisoners[0] = m_lpiPrisoners[1] = 0;
	m_lpAnalyzingEnd = m_lpAnalyzingStones;
	m_lpemLivingMove = &m_emBlankMove;
	m_emBlankMove.test = false;
	m_nHandicap = 0;
	m_iHandicapPutting = 0;
}
